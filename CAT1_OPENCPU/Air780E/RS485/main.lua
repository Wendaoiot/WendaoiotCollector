...
select_data=""
-- -----------MQTT全局参数,根据自己的服务器修改-------------------
local mqtt_host = "api.wendaoiot.com" --"lbsmqtt.airm2m.com"  api.wendaoiot.com
local mqtt_port = 1883
local mqtt_isssl = false
local client_id =(rtc.get().toHex)
local user_name = "test"
local password = "test"
local ca_file=false
local pub_topic = "/server/" .. mobile.imei() --(mcu.unique_id():toHex())
local sub_topic = "/device/" .. mobile.imei()--(mcu.unique_id():toHex())
...
sys.taskInit(function()
    sys.wait(7000)
        ...
        elseif enter_mode==2 then
            ...
            --sys.publish("start_mqtt")
			while 1 do 
				uart.write(uartid_2, "test")
				local tmp = uart.read(uartid_2)
				uart.write(uartid_2, tmp)
				sys.wait(1000)
            end
